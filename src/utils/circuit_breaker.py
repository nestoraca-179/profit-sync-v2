...
class CircuitBreaker:
    ...
    def allow_request(self) -> bool:
        ...
        if self.state == CircuitBreakerState.OPEN and self.opened_at is not None:
            if utc_now() >= self.opened_at + timedelta(seconds=self.timeout_seconds):
                self.state = CircuitBreakerState.HALF_OPEN
                return True
            return False
        if self.state == CircuitBreakerState.HALF_OPEN and self.opened_at is not None:
            if utc_now() >= self.opened_at + timedelta(seconds=self.half_open_timeout_seconds):
                return True
